...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ｓｉｎｚ\n'
            + 'ORG:Owner Nightbot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285261991545:+62 831-1800-241\n'
            + 'END:VCARD'

prefix = "!"
name = "~ NIGHT BOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN NIGHTBOT😁"
rmenu = "HAI TEMEN NIGHTBOT👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 10
memberLimit = 50
ban = []
premium = ["6285261991545@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} WE ALL MISS YOU:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285261991545@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen NIGHTBOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case 'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nama Bot* : ${me.name}\n➽ *Owner Bot* : @${ownerNumber}\n➽ *prefix* : | ${prefix} |\n➽ *Total Block* : ${blocked.length}\n➽ *Aktif Sejak* : ${kyun(uptime)}\n\n➽ *Total Pengguna* : 7895${user.length} User\n➽ *Instagram* : https://www.instagram.com/syarief_mdn\n➽ *Special Thanks To* :\n\n➽ Allah SWT \n➽ MhankBarBar\n➽ Nurutomo\n➽ Monurios`
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6285261991545@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat NIGHTBOT\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *PENGUMUMAN NIGHTBOT* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
